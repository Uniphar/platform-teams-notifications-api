...
public sealed class ServiceBusTeamsCardEventPublisher : ITeamsCardEventPublisher, IAsyncDisposable
{
    ...
    {
        var ns = configuration["FullyQualifiedNamespaceServiceBus"]
            ?? throw new InvalidOperationException("FullyQualifiedNamespaceServiceBus is required");
        _tenantId = configuration["AZURE_TENANT_ID"]
    ...
    public Task PublishCardUpdatedAsync(string uniqueId, string teamsDeepLink, string cardType, bool hasActionableFile, CancellationToken token)
        => SendAsync(new TeamsCardUpdatedCommand
        {
            ...
            HasActionableFile = hasActionableFile,
            TenantId = _tenantId
        }, "TeamsCardUpdated", token);

    public Task PublishCardDeletedAsync(string uniqueId, CancellationToken token)
        => SendAsync(new TeamsCardDeletedCommand { UniqueId = uniqueId, TenantId = _tenantId }, "TeamsCardDeleted", token);

    public Task PublishCardResolvedAsync(string uniqueId, CancellationToken token)
        => SendAsync(new TeamsCardResolvedCommand { UniqueId = uniqueId, TenantId = _tenantId }, "TeamsCardResolved", token);
